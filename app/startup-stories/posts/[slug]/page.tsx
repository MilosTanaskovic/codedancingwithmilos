...
import { getJavaScriptBlogPosts, getStartupStores } from "@/components/mdx/utils";
...
export async function generateStaticParams() {
  const allBlogs = getStartupStores();
...
export async function generateMetadata({
  ...
  const post = getStartupStores().find(
  ...
  return (
    <div className="grow md:flex space-y-8 md:space-y-0 md:space-x-8 pt-12 md:pt-16 pb-16 md:pb-20">
      ...
      <RightSidebar>
        ...
        {/* <WidgetSponsor /> */}
        <WidgetPosts
          allBlogs={getStartupStores().map((post) => ({
            ...post,
            metadata: {
              ...post.metadata,
              mainUrl: post.metadata.mainUrl || "",
